      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 overflow-y-auto pr-2">
          ...
          <div className="relative h-24 mb-12">
            <button type="button" onClick={() => bannerInputRef.current?.click()} className="relative h-full w-full rounded-lg bg-white/10 overflow-hidden group">
              {form.banner_url ? <img src={form.banner_url} alt="Banner" className="w-full h-full object-cover"/> : <div className="w-full h-full"/>}
            ...
            <div className="absolute -bottom-10 left-1/2 -translate-x-1/2 w-24 h-24 rounded-full border-4 border-background bg-background">
              <button type="button" onClick={() => avatarInputRef.current?.click()} className="relative w-full h-full rounded-full overflow-hidden group">
          ...
          {(uploading) && (
            <div className="w-full text-center">
              <p className="text-xs text-accent-cyan animate-pulse">Uploading {uploading?.replace('_url','')}...</p>
            </div>
          )}
