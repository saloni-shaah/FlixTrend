...
import { categorizeContent } from "@/ai/flows/categorize-content-flow";
...
export async function runCategorizationAction(input: z.infer<typeof CategorizationInputSchema>) {
    try {
        const result = await categorizeContent(input);
        return { success: result, failure: null };
    } catch (error: any) {
        console.error("Content categorization action error:", error);
        return { success: null, failure: error.message || "An unknown error occurred during categorization." };
    }
}
