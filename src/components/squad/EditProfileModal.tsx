...
export function EditProfileModal({ profile, onClose }: { profile: any; onClose: () => void }) {
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>, type: 'avatar' | 'banner') => {
        if (e.target.files && e.target.files[0]) {
            ...
            if (file.size > 10 * 1024 * 1024) { // 10MB pre-compression limit
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4 overflow-y-auto pr-2 max-h-[80vh]">
                    <div className="flex items-center gap-4">
                        <div className="relative group shrink-0">
                            ...
                            <input type="file" ref={avatarInputRef} onChange={(e) => handleFileChange(e, 'avatar')} accept="image/*" className="hidden" disabled={isCompressing}/>
                        </div>
                        <div className="relative group w-full h-24 rounded-lg bg-black/20 overflow-hidden">
                            ...
                            <button type="button" onClick={() => bannerInputRef.current?.click()} className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                             ...
                             <input type="file" ref={bannerInputRef} onChange={(e) => handleFileChange(e, 'banner')} accept="image/*" className="hidden" disabled={isCompressing}/>
                        </div>
                    </div>
                    {error && <p className="text-red-400 text-xs my-2 text-center">{error}</p>}
                    {isCompressing && <p className="text-accent-cyan text-xs my-2 text-center animate-pulse">Compressing image...</p>}
                    ...
                    <div className="flex justify-end gap-3 mt-4">
                        ...
                        <button type="submit" className="btn-glass bg-accent-pink text-white" disabled={loading || isCompressing}>
                            {loading ? "Saving..." : isCompressing ? "Compressing..." : "Save Changes"}
                        </button>
                    </div>
                </form>
            </motion.div>
